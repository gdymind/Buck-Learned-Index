...
namespace buckindex {
    ...
    TEST(LinearModel, get_endpoints_model) {
        ...
        EXPECT_LT(abs(m.get_slope() - 0.5), 1e-6);
        EXPECT_LT(abs(m.get_offset() - 0.0), 1e-6);

        LinearModel<uint64_t> m2 = LinearModel<uint64_t>::get_endpoints_model({10, 26});
        EXPECT_LT(abs(m2.get_slope() - 1.0/16), 1e-6);
        EXPECT_LT(abs(m2.get_offset() - -10.0/16), 1e-6);
    }
    ...
    TEST(LinearModel, get_regression_model) {
        ...
        EXPECT_LT(abs(m3.get_slope() - 0.09029), 1e-2);
        EXPECT_LT(abs(m3.get_offset() - -0.7455), 1e-2);
    }
}
