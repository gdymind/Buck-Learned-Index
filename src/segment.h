...
public:
    ...
    Segment(size_t num, Bucket<KeyValueList<T, V,  SBUCKET_SIZE>, T, V, SBUCKET_SIZE>* list, T invalid_pivot, bool leaf = false, Segment* parent = nullptr){
        num_bucket_ = num;
        sbucket_list_ = new Bucket<KeyValueList<T, V,  SBUCKET_SIZE>, T, V, SBUCKET_SIZE>[num]{invalid_pivot};
        is_leaf_ = leaf;
        parent = nullptr;
    }
